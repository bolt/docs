...
There are three files that contain the controller collections: `app/app_backend.php`, `app/app_frontend.php` and `app/app_async.php`. The routes in `app_backend.php` are all pretty straightforward. The ones in `app_frontend.php` are a bit more exotic, since they get matched based on the contenttypes that the developer has set up. 
...
Basically, everything that you can do 'vanilla' Twig, you can do in the Bolt templates. We've added a few tags of our own. Browse `app/source/Bolt/TwigExtension.php` and `app/source/Bolt/SetcontentTokenParser.php` for details. 
...
The "Model"
-----------
The way Bolt handles its contenttypes is defined in the `contenttypes.yml`, which in turn determines the data-structure of the website. Basically, whatever is defined in the contenttypes gets 
