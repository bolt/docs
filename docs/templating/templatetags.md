...
Use this function to create a link to a controller to another page that's 
generated by Bolt. Instead of creating hardcoded links to `/` or `/pages`, 
it's advised to use the `{{ path() }}` function to create a working link to the
page, taking into account the configuration of the web server. Doing this
ensures the links will still work if the site gets installed in a subfolder.
